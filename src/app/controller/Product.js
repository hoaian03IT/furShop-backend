...
class Product {
  async createProduct(req, res) {
    try {
      ...
      const quantity = attributes?.reduce(
        (acc, curr) => acc + curr.quantity,
        0
      );

      const newAttributes = (
        await ProductAttributeModel.insertMany(attributes)
      ).map((attr) => attr._id);
      ...
      res
        .status(200)
        .json({ title: "Thành công", message: "Tạo sản phẩm thành công" });
    } catch (error) {
  ...
  async getProduct(req, res) {
    try {
      ...
      const product = await ProductModel.findById(id)
        ...
        .select(
          "productName price rate description brand category attributes discount"
        );
